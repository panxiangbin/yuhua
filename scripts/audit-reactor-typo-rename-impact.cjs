...
const records = specs.flatMap((spec, index) => {
  ...
  for (const file of textFiles) {
    ...
    for (const value of Object.values(fields)) {
      if (!value || !value.includes(TYPO)) continue;
      const count = countOccurrences(file.content, value);
      if (count) {
        occurrences += count;
        matchedValues.push(value);
      }
    }
    if (!occurrences && file.content.includes(TYPO)) {
      occurrences = countOccurrences(file.content, TYPO);
      if (occurrences) matchedValues.push(TYPO);
    }
    if (occurrences) {
      references.push({
        file: file.relativePath,
        occurrences,
        matchedValues: Array.from(new Set(matchedValues)).join(" | ")
      });
    }
  }
  ...
  return [{
    ...
    referenceOccurrenceCount: references.reduce((sum, item) => sum + item.occurrences, 0),
    referenceFiles: references.map((item) => `${item.file} (${item.occurrences})`).join(" | "),
    risk,
    blockers: blockers.join(" | "),
    recommendation: blockers.length
      ? "先人工处理冲突或缺失资源，再同步修正标题、页面路径、下载路径和全部引用"
      : "可进入人工批准后的原子重命名计划；必须在同一提交中同步更新标题、文件路径和全部引用"
  }];
});
...
const summary = {
  ...
  referencedFiles: new Set(records.flatMap((record) => record.referenceFiles.split(" | ").filter(Boolean))).size,
  automaticRenames: 0,
  safetyRule: "本脚本只生成重命名影响清单，不修改规格书标题、HTML文件、下载文件、产品参数、分类或联系方式",
  recommendation: "人工批准后再采用单次原子提交同步重命名资源与引用，并运行静态资源、跳转、桌面端和手机端测试"
};
...
const headers = [
  ...
  "当前下载存在", "建议下载冲突", "引用文件数", "引用次数", "引用位置", "风险", "阻断事项", "建议处理"
];
const rows = [headers.map(csvCell).join(",")].concat(records.map((record) => [
  ...
  record.referenceFileCount, record.referenceOccurrenceCount, record.referenceFiles,
