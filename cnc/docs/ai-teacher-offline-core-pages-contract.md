...
1. 当前开发分支的目标离线核心契约完整、可测试，起点测评关键安全项硬门禁可冷离线使用，且没有降低既有 AI CNC老师离线能力；
...
- 分支目标 PWA 构建：`20260802-pwa7`；
- 上一正式 PWA 构建：`20260802-pwa6`；
...
这只表示分支尚待合并或部署，不代表 pwa7 已上线。此时必须同时满足：

- 当前分支完整符合 pwa7 十项核心资源契约；
- 远程 `main` 与 Pages 公网仍逐字节、SHA-256 完全一致；
- 公网 pwa6 继续完整符合十项离线核心契约。

`main` push 或手动正式验收时不允许继续存在分支与 `main`/Pages 不一致。正式验收只能在 Pages 真实返回 pwa7 且四项资源与 `main` 完全一致后通过。

除 pwa6、pwa7 外的构建号均视为未受控版本并失败。

## pwa7核心缓存

pwa7 Service Worker 核心缓存必须按受控顺序包含10项资源：
...
pwa6 公网基线也必须核对相同10项资源，不允许为了兼容分支而跳过起点测评、AI CNC老师、现场问诊单或判断说明页。

PWA 状态页和自检页必须与各自构建标记一致。pwa7 自检页必须真实核对10项资源，并明确带关键安全项硬门禁的起点测评可冷离线打开；不得只修改显示数字、重复条目或通过数组长度伪造完整性。
