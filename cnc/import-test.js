...
(function(){
  ...
  function loadScript(src,attr,ready){if(ready&&ready())return Promise.resolve(true);var key='__CNC_LOAD_'+attr.toUpperCase().replace(/-/g,'_')+'__';if(window[key])return window[key];window[key]=new Promise(function(resolve){var old=document.querySelector('script[data-'+attr+']');if(old){if(old.dataset.loaded==='true'){resolve(true);return}old.addEventListener('load',function(){old.dataset.loaded='true';resolve(true)},{once:true});old.addEventListener('error',function(){resolve(false)},{once:true});return}var s=document.createElement('script');s.src=src;s.async=false;s.setAttribute('data-'+attr,'true');s.onload=function(){s.dataset.loaded='true';resolve(true)};s.onerror=function(){console.error('[CNC加载失败]',src);resolve(false)};document.head.appendChild(s)});return window[key]}
  function loadAll(){ensureStyle();return loadScript(EXTENDED_SCRIPT,'learning-images-04-12',function(){return!!window.CNC_LEARNING_VECTOR_POSTERS}).then(function(){return loadScript(PRO_SCRIPT,'cnc-mobile-gcode-pro',function(){return window.__CNC_GM_PRO_INSTALLED__===BUILD})}).then(function(){return loadScript(ALIGN_SCRIPT,'cnc-learning-alignment',function(){return window.__CNC_LEARNING_ALIGNED__===BUILD})})}
  ...
  window.CNC_IMPORT_TEST={runAll:function(){var r={passed:true,build:BUILD,lessons:{},mobileGcodePro:window.__CNC_GM_PRO_INSTALLED__===BUILD,learningAligned:window.__CNC_LEARNING_ALIGNED__===BUILD,openLessonDecorated:!!document.querySelector('#study-detail-content .lesson-image-flow')};for(var n=2;n<=12;n++){var l=lesson(n),c=l&&Array.isArray(l.imageCards)?l.imageCards.length:0;r.lessons[n]=c;if(c<2)r.passed=false}if(!r.mobileGcodePro||!r.learningAligned)r.passed=false;console.log('[CNC综合检查 V5]',r);return r}};
})();
