...
async function openControlledNavigation(context, controlledUrl, observePage, errors) {
  const diagnostics = [];
  ...
  throw new Error(`Service Worker active but fresh navigations were not controlled: ${JSON.stringify(diagnostics)}`);
}
...
async function ensureControlled(page, errors, observePage, options = {}) {
  ...
  await page.close();
  return openControlledNavigation(context, controlledUrl, observePage, errors);
}
