...
(async()=>{
  ...
  try{
    await page.goto('http://127.0.0.1:4173/cnc/course-tool-basics.html',{waitUntil:'networkidle'});
    await page.evaluate(()=>localStorage.clear());
    await page.reload({waitUntil:'networkidle'});
    const body=await page.locator('body').innerText();
    for(const text of ['刀具基础','预计 40 分钟','刀具直径','刃长','伸出量','刀长补偿','立铣刀','钻头','面铣刀','G43 H03','原厂手册','现场条件']){
      if(!body.includes(text)) throw new Error(`missing ${text}`);
    }
    const steps=await page.locator('.step').count();
    if(steps<11) throw new Error(`expected course steps >=11, got ${steps}`);

    await answerQuiz(correctChoices);
    if((await page.locator('#finalScore').innerText())!=='100分') throw new Error('final score is not 100');
    if(!(await page.locator('#resultText').innerText()).includes('已通过第7关')) throw new Error('100-point pass message missing');
    ...
    if(!passedStorage.profile.completedLessons?.includes('tool-basics')) throw new Error('course completion missing');
    if(passedStorage.profile.lessonScores?.['tool-basics']!==100) throw new Error('best score missing');
    ...
    if(failedStorage.profile.completedLessons?.includes('tool-basics')) throw new Error('70-point attempt incorrectly completed course');
    if(failedStorage.profile.lessonScores?.['tool-basics']!==70) throw new Error('70-point best score missing');
    ...
    await page.screenshot({path:'artifacts/tool-basics-success.png',fullPage:true});
    console.log('CNC tool basics course smoke passed: 100-point pass and 70-point fail paths verified');
  }catch(error){
    await page.screenshot({path:'artifacts/tool-basics-failure.png',fullPage:true});
    fs.writeFileSync(path.join('artifacts','tool-basics-error.txt'),`${error.stack||error}\n\nConsole errors:\n${errors.join('\n')}`);
    throw error;
