...
const PREVIOUS_MAIN_PWA = '20260812-pwa39';
const PREVIOUS_MAIN_CACHE = '20260812-learning39';
...
function walk(dir) {
  ...
  return fs.readdirSync(dir, { withFileTypes: true }).flatMap(entry => {
    ...
    if (entry.isDirectory()) {
      if (['node_modules', 'test-results', '.git', 'docs'].includes(entry.name)) return [];
      return walk(full);
    }
    return /\.(?:cjs|js|html|json|md|ya?ml)$/.test(entry.name) ? [full] : [];
  });
}
...
for (const file of files) {
  ...
  if (TRANSITION_FILES.includes(relative)) {
    next = next
      .replace(
        new RegExp(`(const\\s+currentMainPwaBuild\\s*=\\s*['"])${PREVIOUS_MAIN_PWA}(['"])`),
        `$1${FROM_PWA}$2`
      )
      .replace(
        new RegExp(`(\\[currentMainPwaBuild\\]\\s*:\\s*['"])${PREVIOUS_MAIN_CACHE}(['"])`),
        `$1${FROM_CACHE}$2`
      );
  }
  if (next !== source) {
...
for (const file of required) {
  if (!changed.includes(file)) throw new Error(`PWA41同步缺少预期文件：${file}`);
}

for (const file of TRANSITION_FILES) {
  const source = fs.readFileSync(path.join(ROOT, file), 'utf8');
  const mainPwaPattern = new RegExp(`const\\s+currentMainPwaBuild\\s*=\\s*['"]${FROM_PWA}['"]`);
  const mainCachePattern = new RegExp(`\\[currentMainPwaBuild\\]\\s*:\\s*['"]${FROM_CACHE}['"]`);
  if (!mainPwaPattern.test(source)) {
    throw new Error(`当前main过渡构建未锁定PWA40：${file}`);
  }
  if (!mainCachePattern.test(source)) {
    throw new Error(`当前main过渡缓存未锁定learning40：${file}`);
  }
}
...
const diff = execFileSync('git', ['diff', '--name-only'], { cwd: ROOT, encoding: 'utf8' })
  ...
  .split('\n')
  .filter(Boolean);
const outside = diff.filter(file => !(file.startsWith('cnc/') || file.startsWith('.github/workflows/cnc-')));
if (outside.length) throw new Error(`发现非CNC范围改动：${outside.join('、')}`);
...
const report = {
  ...
  toCache: TO_CACHE,
  currentMainPwaBuild: FROM_PWA,
  currentMainCacheRevision: FROM_CACHE,
  changedFiles: diff,
  changedCount: diff.length,
  transitionFiles: TRANSITION_FILES
};
fs.mkdirSync(path.join(ROOT, 'artifacts/pwa41-pin-sync'), { recursive: true });
fs.writeFileSync(path.join(ROOT, 'artifacts/pwa41-pin-sync/report.json'), JSON.stringify(report, null, 2));
