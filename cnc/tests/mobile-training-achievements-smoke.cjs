...
(async () => {
  ...
  await page.waitForFunction(() => window.CNC_TRAINING_ACHIEVEMENTS?.build === '20260724b');
  ...
  assert.match(await links.nth(0).getAttribute('href'), /training-certificate\.html/);
  assert.match(await links.nth(1).getAttribute('href'), /training-calendar\.html/);
  assert.match(await links.nth(2).getAttribute('href'), /training-badges\.html/);
  ...
  console.log('成长成果统计、阶段证书入口、7天预览、下一里程碑和安全提示通过', snapshot);
  await browser.close();
