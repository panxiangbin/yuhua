...
(async () => {
  ...
  assert.equal(api.questions, 60, '高级练习必须覆盖固定12关共60道专属题');
  assert.equal(api.lessonGates, 12);
  assert.equal(api.passScore, 80);
  assert.equal(api.gateVersion, 2, '高级练习必须使用新版关键题硬门禁');
  ...
  console.log('真实学习底栏、60题程序补空、步骤排序、看程序找错、关键题硬门禁与12关绑定检查通过', { api, studyTarget });
  await browser.close();
