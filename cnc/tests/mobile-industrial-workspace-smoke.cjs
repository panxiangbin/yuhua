...
(async () => {
  ...
  const settledSearch = await page.evaluate(() => {
    ...
    return {
      ...
      detailOpen: document.body.getAttribute('data-cnc-detail-open') === 'true' || Boolean(detailPanel && detailPanel.classList.contains('mobile-open')),
  ...
  if (settledSearch.detailOpen) {
    ...
    await page.waitForFunction(() => document.body.getAttribute('data-cnc-detail-open') === 'true' || document.getElementById('detail-panel')?.classList.contains('mobile-open'), null, { timeout: 15000 });
  }
