...
(async()=>{
  ...
  try{
    ...
    for(const text of ['学习目标','尺寸基准','工艺基准','公差','加工余量','孔距','原厂手册','现场条件','受控版本图纸','交叉确认']) assert.ok(bodyText.includes(text),`missing ${text}`);
    const conflictQuestion=await page.evaluate(()=>({q:questions[9].q,opts:questions[9].opts,e:questions[9].e}));
    const conflictText=[conflictQuestion.q,...conflictQuestion.opts,conflictQuestion.e].join(' ');
    assert.match(conflictText,/暂停编程|停止.*确认/,'course must tell learners to stop and verify conflicting drawing information');
    assert.match(conflictText,/不能.*猜|自行猜/,'course must forbid guessing when drawing information conflicts');
