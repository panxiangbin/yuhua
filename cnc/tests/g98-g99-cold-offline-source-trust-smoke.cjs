...
(async () => {
  ...
  try {
    ...
    console.log('CNC G98/G99冷离线源可信度门禁通过：PWA37首次安装后，安全归一化源与G/M基础目录可从静态缓存冷离线读取；关闭本地HTTP源站后网络探针不再到达服务器，离线重载仍保持G98/G99车铣双语义边界、高风险标记和原厂手册核对要求；仅记录冷离线阶段浏览器预期的504(Offline)资源噪声，任何其它控制台错误或关键资源504仍会失败。');
  } catch (error) {
