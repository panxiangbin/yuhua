...
(async () => {
  ...
  await page.waitForFunction(() => window.CNC_TRAINING_PRACTICE && window.CNC_TRAINING_PRACTICE.build === '20260723b', null, { timeout: 20000 });
  ...
  assert.equal(api.questions, 6);
  assert.equal(api.state.version, 1);

  const entry = page.locator('#xp-practice-entry');
  assert.match((await entry.locator('h4').textContent()) || '', /答错就进入错题本/);
  const entryLayout = await entry.evaluate(node => {
    const rect = node.getBoundingClientRect();
    const button = node.querySelector('[data-practice-open]').getBoundingClientRect();
    return { width: rect.width, buttonHeight: button.height };
  });
  ...
  await page.waitForFunction(() => {
    ...
    return state && state.wrong && state.wrong.includes('safe-stop-first');
  });
  ...
  await page.waitForFunction(() => {
    const state = JSON.parse(localStorage.getItem('cnc_training_practice_v1') || 'null');
    return state && state.correct && state.correct.includes('safe-stop-first') && !state.wrong.includes('safe-stop-first');
  });
  ...
  assert.equal(saved.practice.correct.length, 1);
  ...
  const optionLayout = await panel.locator('.xp-practice-options').evaluate(node => {
    ...
    return {
      ...
      minHeight: Math.min(...cards.map(rect => rect.height))
    };
  });
  ...
  console.log('CNC新手在线练习、解析、错题记录、重做与XP防重复通过', {
    build: api.build,
    questions: api.questions,
    correct: saved.practice.correct.length,
    xp: saved.profile.xp
  });
  await browser.close();
