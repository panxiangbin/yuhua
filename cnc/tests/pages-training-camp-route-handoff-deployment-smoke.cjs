...
function parseBuildInfo(text, label) {
  ...
  if (data.pwaBuild === expectedPwaBuild) {
    requireTokens(stage, label, [
      '起点测评关键安全门禁',
      '起点测评离线核心',
      '测评路线一次性交接',
      '训练营路线离线核心',
...
(async () => {
  ...
  try {
    ...
    if (!branchDeploymentPending && publicPwaBuild !== expectedPwaBuild) throw new Error(`分支与main一致时公网必须已经是${expectedPwaBuild}`);
