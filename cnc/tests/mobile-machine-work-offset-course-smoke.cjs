...
(async()=>{
  ...
  const correctChoices=[0,0,[0,1,2],1,0,[0,1,2],1,0,[0,1,2],0];
  ...
  try{
    ...
    for(const text of ['机床坐标与工件坐标','预计 35 分钟','机床坐标系','参考点','工件坐标系','工件零点','坐标偏置','G54','原厂手册','现场条件']){
    ...
    if(!(await page.locator('#resultText').innerText()).includes('已通过第5关')) throw new Error('100-point pass message missing');
    ...
    if(!passedStorage.profile.completedLessons?.includes('machine-work-offset')) throw new Error('course completion missing');
    if(passedStorage.profile.lessonScores?.['machine-work-offset']!==100) throw new Error('best score missing');
    ...
    await page.evaluate(()=>localStorage.clear());
    await page.reload({waitUntil:'networkidle'});
    const seventyPointChoices=[...correctChoices.slice(0,7),1,3,1];
    ...
    if(failedStorage.profile.completedLessons?.includes('machine-work-offset')) throw new Error('70-point attempt incorrectly completed course');
    if(failedStorage.profile.lessonScores?.['machine-work-offset']!==70) throw new Error('70-point best score missing');
    ...
    console.log('CNC machine/work coordinate course smoke passed: 100-point pass and 70-point fail paths verified');
  }catch(error){
