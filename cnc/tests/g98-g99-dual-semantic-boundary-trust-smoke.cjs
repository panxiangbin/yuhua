...
if (info.pwaBuild !== '20260810-pwa35' || info.cacheRevision !== '20260810-learning35') errors.push(`构建版本错误：${info.pwaBuild}/${info.cacheRevision}`);
if (!String(info.contentStage || '').includes('G98/G99车铣双语义适用范围')) errors.push('build-info缺少G98/G99内容可信度阶段');
if (!swText.includes("const BUILD = '20260810-pwa35'") || !swText.includes("const CACHE_REVISION = '20260810-learning35'")) errors.push('Service Worker未对齐PWA33/learning33');
...
if (!guard || guard.version !== 'g10-g28-g53-g92-g94-g95-g96-g97-g98-g99-boundary-8') errors.push(`运行时安全守卫版本错误：${guard?.version}`);
