...
(async () => {
  ...
  try {
    ...
    if (!cachesBefore.includes(`cnc-static-${CACHE_REVISION}`)) {
      throw new Error(`PWA34静态缓存缺失：${JSON.stringify(cachesBefore)}`);
    }
    ...
    await page.goto('http://127.0.0.1:4173/cnc/index.html', { waitUntil: 'domcontentloaded' });
    if (!(await page.title()).includes('CNC')) throw new Error('PWA34冷离线重载首页失败');
    ...
    console.log('CNC G96/G97真实冷离线源可信度门禁通过：PWA34首次安装后，关闭本地HTTP源站并进入离线模式，G96/G97安全归一化源与G/M基础目录仍可从静态缓存读取；冷离线重载继续保留当前CNC、原厂手册、单位制、S含义、最高允许主轴转速及卡盘/装夹/工件/刀具限制，固定教学数值不得直接照抄上机。');
  } catch (error) {
