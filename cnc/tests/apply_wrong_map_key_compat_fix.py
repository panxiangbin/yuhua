...
replace_once(
    'cnc/profile.html',
    "function listValues(v){return Array.isArray(v)?v:record(v)?Object.values(v):[]}",
...
replace_once(
    'cnc/profile.html',
    "function wrongQuestionId(w){return scalarText(w.questionId??w.id??w.key)}function wrongSourceId(w){const hint=scalarText(w.practiceId??w.source??w.setId??w.practice??w.redoUrl).toLowerCase();const direct=SETS.find(id=>hint.includes(id));if(direct)return direct;const id=wrongQuestionId(w).toLowerCase(),mapped=SOURCE_PREFIXES.find(([prefix])=>id.startsWith(prefix));return mapped?mapped[1]:''}",
    "function wrongQuestionId(w,fallback=''){return scalarText(w.questionId??w.id??w.key??fallback)}function wrongSourceId(w,fallback=''){const hint=scalarText(w.practiceId??w.source??w.setId??w.practice??w.redoUrl).toLowerCase();const direct=SETS.find(id=>hint.includes(id));if(direct)return direct;const id=wrongQuestionId(w,fallback).toLowerCase(),mapped=SOURCE_PREFIXES.find(([prefix])=>id.startsWith(prefix));return mapped?mapped[1]:''}"
)
replace_once(
    'cnc/profile.html',
    "function wrongRows(d){const map=new Map;[...listValues(d.wrongQuestions),...listValues(d.wrongItems),...listValues(d.wrong)].forEach(w=>{if(!record(w))return;const sid=wrongSourceId(w),id=wrongQuestionId(w);if(!sid||!id)return;const key=`${sid}:${id}`,old=map.get(key)||{};map.set(key,{...old,...w})});return [...map.values()]}",
    "function wrongRows(d){const map=new Map;[...wrongEntries(d.wrongQuestions),...wrongEntries(d.wrongItems),...wrongEntries(d.wrong)].forEach(([fallback,w])=>{if(!record(w))return;const sid=wrongSourceId(w,fallback),id=wrongQuestionId(w,fallback);if(!sid||!id)return;const key=`${sid}:${id}`,old=map.get(key)||{};map.set(key,{...old,...w,practiceId:sid,questionId:id})});return [...map.values()]}"
)

# 跨专项错题页：对象 key 也是正式兼容的题目 ID 来源，不能 Object.values 后丢失。
replace_once(
    'cnc/practice-wrong-review.html',
    "function listValues(v){if(Array.isArray(v))return v;if(isRecord(v))return Object.values(v);return []}function rawWrong(d){return [...listValues(d.wrongQuestions),...listValues(d.wrongItems),...listValues(d.wrong)]}",
...
replace_once(
    'cnc/practice-wrong-review.html',
    "function questionId(w){return scalarText(w.questionId??w.id??w.key,'')}function sourceId(w){const hint=scalarText(w.practiceId??w.source??w.setId??w.practice??w.redoUrl,'').toLowerCase();const direct=Object.keys(SOURCES).find(id=>hint.includes(id));if(direct)return direct;const id=questionId(w).toLowerCase(),mapped=SOURCE_PREFIXES.find(([prefix])=>id.startsWith(prefix));return mapped?mapped[1]:''}function normalize(d){const map=new Map;rawWrong(d).forEach(w=>{if(!isRecord(w))return;const sid=sourceId(w),id=questionId(w);if(!sid||!id)return;const k=`${sid}:${id}`,old=map.get(k)||{};map.set(k,{sourceId:sid,questionId:id,title:scalarText(w.question??w.title??w.prompt,`错题 ${id}`),ability:scalarText(w.ability??w.dimension??w.category,'待分类'),risk:scalarText(w.risk??w.riskLevel,'待核验'),explanation:scalarText(w.explanation??w.explain??w.analysis??w.reason,'请返回来源专项查看完整解析。'),mergedRecords:Number(old.mergedRecords||0)+1,lastAt:scalarText(w.updatedAt??w.createdAt??w.time??old.lastAt,'')})});return [...map.values()]}",
...
print('ok: profile and wrong-review now preserve object-map question IDs; tests cover key-only records')
