  ...
  try {
    ...
    const snapshot = await page.evaluate(async () => {
      ...
      const workerVersion = await new Promise((resolve, reject) => {
        ...
        channel.port1.onmessage = event => {
          ...
          resolve({
            ...
            cacheRevision: event.data?.cacheRevision || ''
          });
        };
      ...
      return {
        ...
        workerBuild: workerVersion.build,
        workerCacheRevision: workerVersion.cacheRevision,
    ...
    fs.writeFileSync(
      path.join(OUT, 'service-worker-snapshot.json'),
      JSON.stringify({ handoffMode, snapshot }, null, 2)
    );
    ...
    assert.ok(snapshot.cacheRevision, 'build-info必须声明缓存修订号');
    assert.equal(
      snapshot.workerCacheRevision,
      snapshot.cacheRevision,
      `Worker与build-info缓存修订号必须一致：${snapshot.workerCacheRevision}/${snapshot.cacheRevision}`
    );
    assert.ok(snapshot.caches.includes(`cnc-static-${snapshot.cacheRevision}`), '缺少当前缓存修订号对应的静态缓存');
    assert.ok(snapshot.caches.includes(`cnc-runtime-${snapshot.cacheRevision}`), '缺少当前缓存修订号对应的运行时缓存');
    ...
    console.log('Service Worker注册、接管、缓存修订号与构建一致性通过', result);
  } finally {
    await browser.close();
  }
})().catch(error => {
