...
TARGETS = {
    ...
    ROOT / 'cnc' / 'tests' / 'mobile-pwa-offline-cache-smoke.cjs',
}
...
def migrate_offline_test(path: Path) -> None:
    text = path.read_text(encoding='utf-8-sig')
    text = require_known(text, "const PWA_BUILD = '20260809-pwa30';", "const PWA_BUILD = '20260809-pwa31';", 'offline test pwa')
    text = require_known(text, "const CACHE_REVISION = '20260809-learning30';", "const CACHE_REVISION = '20260809-learning31';", 'offline test cache')
    if "stage = 'cold-offline-g92-directory';" not in text:
        marker = "    stage = 'cold-offline-main-learning-content';"
        if marker not in text:
            raise SystemExit('offline test: 找不到G92插入位置')
        block = """    stage = 'cold-offline-g92-directory';\n    const offlineG92Trust = await page.evaluate(async () => {\n      const response = await fetch('./gm-code-complete.js');\n      return { ok: response.ok, text: await response.text() };\n    });\n    if (!offlineG92Trust.ok) throw new Error('G92可信目录首次安装后冷离线读取失败');\n    for (const token of ['车铣差异', '部分铣床/加工中心', '部分车床', '当前CNC与机床厂原厂手册', 'G52/G54-G59', 'X/Z或U/W', 'I/Q/F', '主轴同步', '安全退刀空间', '两类程序不能直接互抄']) {\n      if (!offlineG92Trust.text.includes(token)) throw new Error(`G92冷离线源目录缺少双语义安全边界：${token}`);\n    }\n    for (const forbidden of ['加工中心/旧系统可用于坐标设定，车床常用于简单螺纹循环。', '车床：G92 X20. Z-30. F1.5 表示螺纹循环。', 'G92就是螺纹循环', 'G92就是坐标设定']) {\n      if (offlineG92Trust.text.includes(forbidden)) throw new Error(`G92冷离线源目录仍含无适用范围表述：${forbidden}`);\n    }\n\n"""
        text = text.replace(marker, block + marker, 1)
    if 'g92DualSemanticTrustColdOffline: true,' not in text:
        marker = '      toolOffsetMappingTrustColdOffline: true,\n'
        if marker not in text:
            raise SystemExit('offline test: 找不到结果字段插入位置')
        text = text.replace(marker, marker + '      g92DualSemanticTrustColdOffline: true,\n', 1)
    for token in ["PWA_BUILD = '20260809-pwa31'", "CACHE_REVISION = '20260809-learning31'", "cold-offline-g92-directory", 'G92冷离线源目录缺少双语义安全边界', 'g92DualSemanticTrustColdOffline: true']:
        if token not in text:
            raise SystemExit(f'offline test: 缺少 {token}')
    path.write_text(text, encoding='utf-8')
...
migrate_offline_test(ROOT / 'cnc' / 'tests' / 'mobile-pwa-offline-cache-smoke.cjs')
print('PWA31状态页、自检页、冷离线与PWA30→PWA31升级数据测试迁移完成')
