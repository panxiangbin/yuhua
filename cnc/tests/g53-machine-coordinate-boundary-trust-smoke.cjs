...
if (buildInfo.pwaBuild !== '20260809-pwa30' || buildInfo.cacheRevision !== '20260809-learning30') {
  errors.push(`build-info与PWA30不一致：${buildInfo.pwaBuild} / ${buildInfo.cacheRevision}`);
}
if (!String(buildInfo.contentStage || '').includes('G53机床坐标定位适用范围')) {
  errors.push('build-info缺少G53机床坐标定位适用范围阶段标记');
}

// 第一层：基础G/M目录本身必须去掉“G53=通用安全回退”“Z0=安全点”等危险简化。
...
// 第二层：标准页面运行时必须维持同一安全边界；运行时保护不能成为掩盖基础源错误的唯一真相源。
...
if (!guard || guard.version !== 'g10-g28-g53-boundary-3' || typeof guard.normalizeG53 !== 'function') {
  errors.push('G10/G28/G53内容安全归一化器未安装、版本异常或缺少normalizeG53');
}
...
console.log('CNC G53机床坐标定位可信度门禁通过：G53被限定为控制器/机床相关的高风险非模态机床坐标运动；机床坐标零点、刀补影响、目标位置与完整运动空间必须按本机原厂手册核对并先做受控空运行，Z0或换刀位置不得作为跨机床通用安全点。');
