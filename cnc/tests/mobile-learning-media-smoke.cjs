...
const edge = process.env.CNC_EDGE_PATH || undefined;
...
(async()=>{
  ...
  try{
    ...
    const media=await page.evaluate(async()=>{
      ...
      if(video){await video.play();await new Promise(r=>setTimeout(r,700));video.pause()}
      return {images:imgs.length,decoded:imgs.filter(img=>img.complete&&img.naturalWidth>0).length,video:!!video,videoWidth:video?video.videoWidth:0,duration:video?video.duration:0,advanced:video?video.currentTime>before:false,demo:!!document.querySelector('#demo-stage svg'),toggle:!!document.querySelector('#demo-toggle'),scrollWidth:document.documentElement.scrollWidth,innerWidth};
    });
    ...
    fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({passed:true,media,errors},null,2));
    console.log(JSON.stringify({passed:true,media}));
  } finally {
