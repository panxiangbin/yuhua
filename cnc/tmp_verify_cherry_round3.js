...
(async() => {
  ...
  const before = await page.evaluate(() => ({
    ...
    quickSearchExists: !!document.querySelector('#quick-search-input')
  }));
  try { await page.click('[data-route="learning-map"]'); } catch (e) {}
  ...
  const learningMap = await page.evaluate(() => ({
    ...
    placeholderText: Array.from(document.querySelectorAll('*')).filter(el => /知识地图|加载中|暂无/.test(el.textContent||'')).slice(0,5).map(el => el.textContent.trim()).join(' | ')
  }));
  ...
  try {
    ...
    if (firstResult) {
      ...
      const recBtn = await page.$('.recommendation-link');
      if (recBtn) {
        await recBtn.click();
        await page.waitForTimeout(1200);
        ...
        recResult = { clickable:false, reason:'no recommendation-link rendered', beforeTitle };
      }
    } else {
      recResult = { clickable:false, reason:'no search result rendered' };
    }
  } catch (e) {
    recResult = { clickable:false, error:String(e) };
  }
  console.log(JSON.stringify({before, learningMap, recResult, errors}, null, 2));
  await page.screenshot({path:'F:/AI工作台/verify_cherry_round3.png', fullPage:true});
  await browser.close();
})();
