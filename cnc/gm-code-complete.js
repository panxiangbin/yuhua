window.CNC_GM_CODES = [
  ...
  },
  {
    ...
    "usage": "仅在确认当前CNC明确支持对应语义后使用；若涉及恒线速度最高转速限制，必须核对本机G50的组别、S地址单位与限制方式；旧程序坐标设定或其它功能也必须按机床厂原厂手册确认。",
    "beginner": "G50含义最容易因机型不同而混淆。",
    "warning": "不同机型或控制器上的G50语义可能不同；不要把G50 Sxxxx当成跨系统通用限速格式。涉及主轴最高转速时，还必须同时满足主轴、卡盘、装夹、工件、刀具和机床配置限制，并核对当前CNC与机床厂原厂手册。",
    "example": "教学语义示意：部分车床CNC中G50可用于设置恒线速度模式下的最高主轴转速限制；具体格式、S单位和上限必须以当前CNC与机床厂原厂手册为准，不提供可直接照抄的固定数值。",
    "risk": "高",
    "tags": [
      "G50",
      "限速",
